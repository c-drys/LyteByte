...
app.post("/restaurant_order", (request, response) => {
  ...
  db.query(updateOrderStatus)
    .then(data => {
      ...
      response.redirect("/restaurant_order")
    })
    .catch(err => {
...
app.get("/restaurant_order", (request, response) => {
  ...
  db.query(getOrder)
  .then(data => {
    response.redirect("/restaurant_order", data)
